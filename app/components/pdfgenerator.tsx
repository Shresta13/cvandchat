...
export function generateModernPDF(data: ResumeData) {
  ...
  doc.text(data.personalInfo.fullName || 'Your Name', mLeft, 16);
  ...
  cLines.forEach((line: string, i: number) => {
    doc.text(line, mLeft, 27 + i * 5);
  });

  y = cLines.length > 1 ? 54 : 54;
...
export function generateTwoColumnPDF(data: ResumeData) {
  ...
  const checkLeftPage  = (n: number) => { if (leftY  + n > pageH - 12) { doc.addPage(); leftY  = 18; rightY = 18; } };
  const checkRightPage = (n: number) => { if (rightY + n > pageH - 12) { doc.addPage(); leftY  = 18; rightY = 18; } };
  ...
  doc.setFontSize(8);
  doc.setTextColor(...GRAY);
  if (data.personalInfo.phone)    { doc.text(data.personalInfo.phone, mPad, leftY);    leftY += 4; }
  if (data.personalInfo.email)    { doc.text(data.personalInfo.email, mPad, leftY);    leftY += 4; }
  if (data.personalInfo.location) { doc.text(data.personalInfo.location, mPad, leftY); leftY += 4; }
  if (data.personalInfo.linkedin) { doc.text(data.personalInfo.linkedin.replace(/^https?:\/\//i,''), mPad, leftY); leftY += 4; }
  if (data.personalInfo.github)   { doc.text(data.personalInfo.github.replace(/^https?:\/\//i,''), mPad, leftY);   leftY += 4; }
  leftY += 2;
  ...
  if (data.personalInfo.summary) {
    ...
    checkRightPage(lines.length * 4.5);
    doc.text(lines, leftW + mPad, rightY);
    rightY += lines.length * 4.5 + 3;
  }
...
export function generateElegantPDF(data: ResumeData) {
  ...
  doc.rect(mLeft, y, contentW, 7, 'F');
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8);
  doc.setTextColor(...GRAY);
  doc.text(contacts, pageW / 2, y + 4.5, { align: 'center' });
  y += 11;
  ...
  if (data.references && data.references.length > 0) {
    ...
    data.references.forEach((ref) => {
      ...
      y += 4;
      if (ref.phone) { doc.setFont('helvetica', 'normal'); doc.setFontSize(8); doc.text(`Phone: ${ref.phone}`, mLeft, y); y += 4; }
      if (ref.email) { doc.text(`Email: ${ref.email}`, mLeft, y); y += 4.5; }
    });
  }
...
export function generateProfessionalPDF(data: ResumeData) {
  ...
  const checkPage = (side: 'left' | 'right', n: number) => {
    ...
    if (y + n > pageH - 12) { doc.addPage(); leftY = 18; rightY = 18; }
  };
  ...
  doc.setFontSize(18);
  doc.setTextColor(255, 255, 255);
  doc.text(data.personalInfo.fullName || 'Your Name', mPad, 15);
  if (data.personalInfo.title) {
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(10);
    doc.setTextColor(200, 200, 200);
    doc.text(data.personalInfo.title, mPad, 23);
  }
  ...
  contactItems.forEach((item, i) => {
    doc.text(item!, pageW - mPad, 13 + i * 7, { align: 'right' });
  });
  ...
  if (data.personalInfo.summary) {
    ...
    checkPage('right', lines.length * 4.5);
    doc.text(lines, leftW + mPad, rightY);
    rightY += lines.length * 4.5 + 3;
  }
  ...
  if (data.references && data.references.length > 0) {
    ...
    data.references.forEach((ref) => {
      ...
      doc.text(`${ref.position || ''}${ref.company ? ` · ${ref.company}` : ''}`, leftW + mPad, rightY);
      rightY += 4;
      if (ref.phone) { doc.setFont('helvetica', 'normal'); doc.setFontSize(8); doc.text(`Phone: ${ref.phone}`, leftW + mPad, rightY); rightY += 4; }
      if (ref.email) { doc.text(`Email: ${ref.email}`, leftW + mPad, rightY); rightY += 4.5; }
    });
  }
...
export function generateDarkSidebarPDF(data: ResumeData) {
  ...
  if (data.personalInfo.email)    { const el = doc.splitTextToSize(data.personalInfo.email, leftW - mPad * 2); doc.text(el, mPad, leftY); leftY += el.length * 4; }
  if (data.personalInfo.location) { doc.text(data.personalInfo.location, mPad, leftY); leftY += 4.5; }
  if (data.personalInfo.linkedin) { const ll = doc.splitTextToSize(data.personalInfo.linkedin.replace(/^https?:\/\//i,''), leftW - mPad * 2); doc.text(ll, mPad, leftY); leftY += ll.length * 4; }
  if (data.personalInfo.github)   { const gl = doc.splitTextToSize(data.personalInfo.github.replace(/^https?:\/\//i,''), leftW - mPad * 2); doc.text(gl, mPad, leftY); leftY += gl.length * 4; }
  leftY += 2;
  ...
  if (data.references && data.references.length > 0) {
    ...
    data.references.forEach((ref) => {
      ...
      leftY += 4;
      if (ref.phone) { doc.text(ref.phone, mPad, leftY); leftY += 4; }
      if (ref.email) { const el = doc.splitTextToSize(ref.email, leftW - mPad * 2); doc.text(el, mPad, leftY); leftY += el.length * 4; }
      leftY += 2;
    });
  }
